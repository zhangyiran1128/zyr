"""
Example: Train a control direction for intervals

This example demonstrates how to train an RFM direction to control
a specific musical note (e.g., interval 6 vs other intervals) in music generation, using the Syntheory dataset.
"""
...
def main():
    ...
    TARGET_INTERVAL = "6"  # The interval we want to control
    ...
    print(f"Training control direction for interval: {TARGET_INTERVAL}")
    ...
    dataset = load_dataset("meganwei/syntheory", "intervals")["train"] # options: notes, chords, scales, time_signatures, intervals, simple_progressions, tempos
    
    # Split data into positive (note C) and negative (other notes)
    print(f"\nPreparing data for interval: {TARGET_INTERVAL}")
    positive_examples = [x for x in dataset if x["interval"] == int(TARGET_INTERVAL)]
    negative_examples = [x for x in dataset if x["interval"] != int(TARGET_INTERVAL)]
    ...
    train_labels = torch.tensor(
        [1 if x["interval"] == int(TARGET_INTERVAL) else 0 for x in train_samples]
    ).reshape(-1, 1)
    val_labels = torch.tensor(
        [1 if x["interval"] == int(TARGET_INTERVAL) else 0 for x in val_samples]
    ).reshape(-1, 1)
    test_labels = torch.tensor(
        [1 if x["interval"] == int(TARGET_INTERVAL) else 0 for x in test_samples]
    ).reshape(-1, 1)
    ...
    # Save the directions
    concept_name = f"interval_{TARGET_INTERVAL}_ncomp{N_COMPONENTS}"
